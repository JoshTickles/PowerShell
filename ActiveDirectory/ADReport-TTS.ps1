<#
.SYNOPSIS
    Generate graphed AIO audit for all Active Directory objects.

.DESCRIPTION
    Generate a graphed report for all Active Directory objects. 
    This gathers data fro AD and shoots it off to the ReportHTML Module. https://www.powershellgallery.com/packages/ReportHTML 

    This script and most heavy lifting credit is due to Bradley Wyatt, however has been editted and modified by Josh to tidy up and change some items for TTS use. 
    It is a slow running script if you have a large directory to set it to run and get a coffee. Once complete you will have a HTML file with all your data inside.
...
.NOTES
    ...
    Date: 12/2018
    Modified: Josh A - TTS internal use for clients
#>
...
# Defaults for the script to use
param (
	
	#Company logo that will be displayed on the left, can be URL or UNC
	#[Parameter(ValueFromPipeline = $true, HelpMessage = "Enter URL or UNC path to client Logo")]
	#[String]$MainLogo = "$clientlogo",

	#TTS Logo that will be on the right side, UNC or URL
	[Parameter(ValueFromPipeline = $true, HelpMessage = "Enter URL or UNC path for TTS Logo")]
	[String]$TTSLogo = "https://www.tts.co.nz/wp-content/uploads/TTS_Master_Colour200x50.png",

	#Title of generated report
	#[Parameter(ValueFromPipeline = $true, HelpMessage = "Enter desired title for report")]
	#[String]$ReportTitle = "$client - Active Directory Report",

	#Location the report will be saved to
	[Parameter(ValueFromPipeline = $true, HelpMessage = "Enter desired directory path to save; Default: C:\TTS\")]
	[String]$ReportSavePath = "C:\TTS\",

	#Find users that have not logged in X Amount of days, this sets the days
	[Parameter(ValueFromPipeline = $true, HelpMessage = "Users that have not logged in within [X] amount of days; Default: 15")]
	$Days = 15,
...
#Title
Write-host "
             _____    _____                       _              _______ _______ _____ 
       /\   |  __ \  |  __ \                     | |            |__   __|__   __/ ____|
      /  \  | |  | | | |__) |___ _ __   ___  _ __| |_   ______     | |     | | | (___  
     / /\ \ | |  | | |  _  // _ \ '_ \ / _ \| '__| __| |______|    | |     | |  \___ \ 
    / ____ \| |__| | | | \ \  __/ |_) | (_) | |  | |_              | |     | |  ____) |
   /_/    \_\_____/  |_|  \_\___| .__/ \___/|_|   \__|             |_|     |_| |_____/ 
                                | |                                                    
                                |_|                                                    "
Write-Host "  AD Structure Audit Report                        Modified for TTS use - Josh A - 2019" -ForegroundColor green
Write-Host ""
Write-Host "Requires - WMF 5.1+"
Write-Host "Requires - Run from DC or Machine with RSAT"
Write-Host "__________________________________" -ForegroundColor White
sleep 3
Write-Host ""
Write-Host "Enter Client Customization as Follows..." -ForegroundColor Red
sleep 1
$Mainlogo = Read-Host -Prompt "1) Enter URL or UNC path to client Logo, otherwise leave blank."
$Client = Read-Host -Prompt "2) Enter name or tag for the client. e.g. MAGS"
$ReportTitle = "$client - Active Directory Report"
sleep 2
Write-Host ""
Write-Host "Gathering Current Report Customization..." -ForegroundColor Red
Write-Host ""
sleep 1
Write-Host "__________________________________" -ForegroundColor White
(Write-Host -NoNewline "Client Logo (left): " -ForegroundColor Yellow),(Write-Host  $MainLogo -ForegroundColor White)
(Write-Host -NoNewline "TTS Logo (right): " -ForegroundColor Yellow),(Write-Host  $TTSLogo -ForegroundColor White)
...
sleep 1
Write-Host ""
Write-Host "Starting script... This will take a while, go get coffee. " -ForegroundColor Red
Write-Host ""
...
#Check for ReportHTML Module and install as required.
...
foreach ($Group in $Groups)
{
	...
	Catch
	{
		write-host -ForegroundColor Yellow "Cannot resolve the manager, " $Manager " on the group " $group.name "- Perhaps one is not entered."
	}
...
$FinalReport.Add($(Get-HTMLOpenPage -TitleText $ReportTitle -LeftLogoString $MainLogo -RightLogoString $TTSLogo))
